...
export default function Register() {
  ...
  function handelRegister() {
    if (!email || !pass || !photo || !name) {
      setErr("All field must fill!");
      return;
    }

    if (!validatePassword(pass)) {
      setErr("Password must have 1 upper & lower case and 6 digite");
      return;
    }
    createUser(email, pass)
      .then(() => {
        updateUserData({ displayName: name, photoURL: photo })
          .then(() => {
            toast.success("Registre Successfull");
          })
          .catch((err) => {
            toast.error("Name image not updated");
          });
      })
      .catch((err) => {
  ...
  return (
    <>
      <section className="mt-7">
        <div
          className={`${
            theme ? "border-2 rounded-lg" : ""
          } relative py-3 sm:max-w-xl sm:mx-auto w-full `}
        >
          <div
            className={`${
              theme ? "bg-[#0f0f0f]" : "bg-white"
            } relative px-4 py-10  mx-8 md:mx-0 shadow-md rounded-3xl sm:p-10`}
          >
